...
function color_show(id, color, obj) {
	...
	$G('destoon_color').style.left = (obj.offsetLeft + leftpos) + 'px';
	$G('destoon_color').style.top = (obj.offsetTop + toppos + 20) + 'px';
	$G('destoon_color').innerHTML = color_htm;
	color_id = id;
	color_bk = color;
	$G('destoon_color').style.display = '';
	if(color) color_view(color);
	color_setup(color);
}
function color_hide() {
	$G('destoon_color').style.display = 'none';
	//Es();
}
function color_close() {
	color_hide();
	$G('color_img_'+color_id).style.backgroundColor = color_bk;
}
function color_select(color) {
	$G('color_input_'+color_id).value = color;
	$G('color_img_'+color_id).style.backgroundColor = color;
	color_hide();
}
function color_setup(color) {
	...
	colors_select += '</table>';
	$G('destoon_color_show').innerHTML = colors_select;
}
function color_view(color) {
	try {
		$G('color_viewview').value = color;
		$G('color_viewview').style.color = color;
		$G('color_img_'+color_id).style.backgroundColor = color;
		$G('title').style.color = color;
	} catch(e) {}
}
//-->